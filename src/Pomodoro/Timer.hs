...
class (KnownNat a) => TimerValue (a :: Nat) 
instance (KnownNat a, a <= 5400) => TimerValue a

data Timer a = (TimerValue a) => MkTimer (Fin a)
...
someTimerValue i = case someNatVal i of
  Just (SomeNat v ) -> case v %<=? (Proxy @5400) of
          LE  Refl -> Just $ MkSomeTimerValue v 
          NLE _ _   -> Nothing
  Nothing -> Nothing

withSomeTimer :: SomeTimer -> (forall a . (TimerValue a) => Timer a -> r) -> r
withSomeTimer (MkSomeTimer v) f = f v

withSomeTimerValue :: SomeTimerValue -> (forall a . (TimerValue a) => Proxy a -> r) -> r
withSomeTimerValue (MkSomeTimerValue v) f = f v

instance Show (Timer a) where
  show (MkTimer f) = "Timer " ++ show f

timer :: forall a . (TimerValue a) => Timer a
timer = MkTimer $ maxBound

finishedTimer :: forall a . (TimerValue a) => Timer a
finishedTimer = MkTimer $ minBound

advance :: Timer a -> Maybe (Timer a)
advance (MkTimer f) = MkTimer <$> predecessor f

timerMax :: forall (a :: Nat) . KnownNat a => Timer a -> Integer
