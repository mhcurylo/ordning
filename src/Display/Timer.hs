...
around a p m = (<> reset <> noblinking) $ (green <>) $ surroundAt p (bright <> red) (reset <> noblinking <> green) . leftPad (p*2 + 1) ' ' . drop (fromInteger $ a - p) . take (fromInteger $ p + a + 1) $ ordningTape m

colorClock :: Padding -> At -> Max -> String
colorClock p a m = "[" ++ around a p m ++ "]"

displayTimer :: KnownNat a => Timer a -> String
displayTimer t = colorClock 10 ((timerValue t + 29) `div` 30) (timerMax t `div` 30)
